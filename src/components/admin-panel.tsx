...
export function AdminPanel({
  ...
  const connectBot = useCallback(async () => {
    ...
    try {
      ...
      if (result.active) {
        ...
        notify(fa.telegramConnectedBanner, "success");
      } else {
        setBotBanner({ tone: "error", text: result.message });
        notify(fa.telegramFailedBanner, "error");
      }
      void loadTelegram();
    } catch (error) {
      const text = error instanceof Error ? error.message : fa.telegramFailedBanner;
  ...
  const toggleGuestUpload = async (next: boolean) => {
    try {
      ...
      notify(next ? "بارگذاری مهمان‌ها فعال شد" : "بارگذاری مهمان‌ها قفل شد", "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : fa.errorGeneric, "error");
    }
  };
  ...
  const saveName = async () => {
    ...
    try {
      ...
      onConfigChange(updated);
      notify(fa.settingsSaved, "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : fa.errorGeneric, "error");
    }
  };
  ...
  const saveProfile = async () => {
    if (!currentPassword) {
      notify(fa.currentPassword, "error");
      return;
    }
    if (newPassword && newPassword !== confirmPassword) {
      notify(fa.passwordsMismatch, "error");
      return;
    }
    if (newPassword && newPassword.length < 8) {
      notify(fa.passwordTooShort, "error");
      return;
    }
    ...
    try {
      ...
      setNewPassword("");
      setConfirmPassword("");
      notify(fa.profileUpdated, "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : fa.errorGeneric, "error");
    } finally {
  ...
  const addContact = async () => {
    ...
    try {
      ...
      setTelegramId("");
      setLabel("");
      notify(fa.whitelistAdded, "success");
    } catch (error) {
      notify(error instanceof Error ? error.message : fa.errorGeneric, "error");
    }
  };
  ...
  const removeContact = async (id: string) => {
    try {
      ...
      setTelegram((prev) => (prev ? { ...prev, whitelist: result.whitelist } : prev));
      notify(fa.whitelistRemoved, "info");
    } catch (error) {
      notify(error instanceof Error ? error.message : fa.errorGeneric, "error");
    }
  };
  ...
  return (
    <Modal
      ...
      wide
      title={fa.adminTitle}
      subtitle={fa.adminSubtitle}
    >
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "قطعه‌ها", value: stats.songCount },
            { label: "لیست‌های پخش", value: stats.categoryCount },
            { label: "محل ذخیره", value: "دیسک محلی" },
            { label: "وابستگی ابری", value: "ندارد" },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <Switch
          ...
          label={fa.allowGuestUpload}
          description={fa.allowGuestUploadHint}
        />

        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <Field label={fa.cafeName}>
            <input
        ...
        <div className="space-y-4 rounded-2xl border border-white/8 bg-black/25 p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-cafe-ink">{fa.credentials}</p>
              <p className="text-xs text-white/45">
                برای هر تغییر، وارد کردن گذرواژهٔ فعلی الزامی است.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label={fa.currentPassword}>
              <input
                className={inputClass}
                type="password"
                value={currentPassword}
                autoComplete="current-password"
                placeholder="برای ذخیره لازم است"
                onChange={(event) => setCurrentPassword(event.target.value)}
              />
            </Field>
            <Field label={fa.newPassword} hint="برای حفظ گذرواژهٔ فعلی، خالی بگذارید.">
              <input
                className={inputClass}
                type="password"
                value={newPassword}
            ...
            <Field label={fa.confirmPassword}>
              <input
                className={inputClass}
                type="password"
                value={confirmPassword}
                autoComplete="new-password"
                placeholder="تکرار گذرواژهٔ جدید"
          ...
          >
            ...
            {profileBusy ? fa.loading : fa.updateProfile}
          </Button>
        </div>
        ...
        <div className="space-y-4 rounded-2xl border border-white/8 bg-black/25 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-cafe-ink">
                  ...
                  {telegram?.botUsername ? (
                    <span data-ltr className="mx-1 text-white/40">
                      @{telegram.botUsername}
                    </span>
                  ) : null}
                </p>
                <p className="text-xs text-white/45">
                  {botRuntime?.message ?? telegram?.message ?? fa.loading}
                </p>
              </div>
            </div>
            <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  ...
                  {botRuntime?.configured
                    ? botRuntime.active
                      ? fa.telegramActive
                      : fa.telegramInactive
                    : fa.telegramNotConfigured}
                </span>
                <span className="text-white/40">{fa.telegramHint}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {botBusy ? fa.telegramConnecting : fa.telegramConnect}
                </Button>
          ...
          <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
            ...
            <input
              className={inputClass}
              placeholder="برچسب (مثلاً سارا — باریستا)"
              value={label}
          ...
          <div className="space-y-2">
            {telegram && telegram.whitelist.length > 0 ? (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-white/40">
                      شناسه: <span data-ltr>{contact.telegramId}</span>
                    </p>
                  </div>
                  <button
                    type="button"
              ...
              <p className="flex items-center gap-2 text-xs text-white/40">
                ...
                {fa.whitelistEmpty} کاربران می‌توانند با ارسال /whoami شناسهٔ خود را ببینند.
              </p>
            )}
          </div>
        </div>
